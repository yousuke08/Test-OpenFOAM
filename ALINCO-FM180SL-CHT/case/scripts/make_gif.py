"""
Generate animated GIF from ALINCO FM180SL chtMultiRegionFoam results.
"""
...
# Fix colour range across all frames
...
air_vmin, air_vmax = 0.0, T_al_all.max()  # clip below-ambient artefacts
...
def make_frame(t):
    ...
    T_air = np.clip(T_air, 0, al_vmax)
    U_air = read_vector("air",      t, "U")

    fig, axes = plt.subplots(1, 2, figsize=(13, 7))
    fig.patch.set_facecolor("#1a1a2e")
    fig.suptitle(
        f"ALINCO FM180SL — CHT + Natural Convection  |  Q=2.5W  |  t={float(t):.0f} s",
    ...
    # --- left: aluminum ---
    ax = axes[0]
    ax.set_title("Aluminum T [deg C]", color='white', fontsize=10)
    if use_tri:
        cs = ax.tricontourf(triang_al, T_al, levels=levels_al,
                            cmap="hot", vmin=al_vmin, vmax=al_vmax, extend='both')
    else:
        cs = ax.scatter(cx_al, cy_al, c=T_al, cmap='hot', vmin=al_vmin, vmax=al_vmax, s=40)
    cb = plt.colorbar(cs, ax=ax)
    cb.ax.yaxis.set_tick_params(color='white')
    plt.setp(cb.ax.yaxis.get_ticklabels(), color='white')
    cb.set_label("T [deg C]", color='white')
    ...
    ax.set_xlabel("x [m]"); ax.set_ylabel("y [m]")
    ax.set_xlim(-0.112, 0.112); ax.set_ylim(-0.005, 0.115)
    ax.set_aspect("equal")
